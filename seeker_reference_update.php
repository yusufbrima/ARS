...
include('include/function_library.php');
  if((isset($_GET['reference_id']) && !empty($_GET['reference_id'])) && (isset($_GET['user_id']) && !empty($_GET['user_id']))){
    $ref_id =  mysql_fix_string(trim($_GET['reference_id']));
    $user_id =  mysql_fix_string(trim($_GET['user_id']));
    $reference_query = "SELECT * FROM seeker_reference WHERE seeker_id='$user_id' AND id='$ref_id'";
       ...
       <div class="container">
          <div class="col-md-6">
              <form method="post" action="<?php outputData($_SERVER['PHP_SELF']); ?>" name="frmSeekerReference" id="frmSeekerReference">
                <fieldset>
                  <legend class="legend">Update Reference</legend>
                  <div class="form-group">
                      <label for="inputFirstName">First Name<em class="required">*</em></label>
                      <input class="form-control" type="text" value="<?php echo($row['first_name']); ?>" name="inputFirstName" id="inputFirstName"
                        placeholder="First Name"  />
                  </div>
                  <div class="form-group">
                      <label for="inputLastName">Last Name<em class="required">*</em></label>
                      <input class="form-control" type="text" value="<?php echo($row['last_name']); ?>" name="inputLastName" id="inputLastName"
                  ...
                  <div class="form-group">
                      ...
                      <input class="form-control" type="text" value="<?php echo($row['organization']); ?>" name="inputOrganization" id="inputOrganization"
                        placeholder="Enter Reference's Organization"  />
                  </div>
                  <div class="form-group">
                        <label for="inputEmail">Email<em class="required">*</em></label>
                        <input class="form-control" type="email" name="inputEmail" id="inputEmail" value="<?php echo($row['email']); ?>"
                          placeholder="Email" autocomplete="on" />
                    </div>
                    <div class="form-group">
                          <label for="inputWebLink">Web URL</label>
                          <input class="form-control" type="url" name="inputWebLink" id="inputWebLink" value="<?php echo($row['web_link']); ?>"
                            placeholder="Enter web link" >
                    </div>
                    <div class="form-group">
                      <label for="inputTelephone">Mobile<em class="required">*</em></label>
                      <input class="form-control" type="tel" value="<?php echo $row['phone']; ?>" name="inputTelephone" id="inputTelephone"
                                        ...
                                        if (isset($_POST['inputTelephone']) && !empty($_POST['inputTelephone'])){
                                          if (isset($_POST['sex']) && !empty($_POST['sex'])){
                                            ...
                                            $reference_url = isset($_POST['inputWebLink'])?mysql_fix_string(trim($_POST['inputWebLink'])):null;
                                              if(update_seeker_reference($con, $reference_first_name,$reference_sex,$reference_last_name,$reference_organization,$reference_email,$reference_telephone,$reference_url,$user_id,$ref_id)){
                  ...
                  <div class="form-group form-inline">
                    <input type="submit" name="save" value="Continue>>" class="btn btn-default btn-primary" />
