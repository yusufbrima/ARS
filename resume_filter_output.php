            ...
            switch ($flag) {
              case '1': //ob_category;
                if(isset($_GET['j_id']) && !empty($_GET['j_id'])){
                  $job_id=mysql_fix_string(trim($_GET['j_id']));
                  $query ="SELECT s.id,s.first_name,s.last_name,s.profile_url,u.join_date,s.sex,s.street,u.user_id FROM seeker AS s LEFT JOIN user AS u ON s.user_id=u.user_id LEFT JOIN  seeker_job_prefrence AS p ON p.seeker_id=s.id WHERE p.job_category={$job_id}";
                  $resume_result = $con->query($query);
                  $content ='<div class="row advert">';
                  $content .="<div class='h2'>Resumes with desired Job Category</div>";
                ...
                break;
              case '2': //employment_type
                  if(isset($_GET['emp_id']) && !empty($_GET['emp_id'])){
                    $emp_id=mysql_fix_string(trim($_GET['emp_id']));
                    $query ="SELECT s.id,s.first_name,s.last_name,s.profile_url,u.join_date,s.sex,s.street,u.user_id FROM seeker AS s LEFT JOIN user AS u ON s.user_id=u.user_id LEFT JOIN  seeker_job_prefrence AS p ON p.seeker_id=s.id WHERE p.employment_type={$emp_id}";
                    $resume_result = $con->query($query);
                    $content ='<div class="row advert">';
                    $content .="<div class='h2'>Resumes with desired Employment Type</div>";
                    $affected_resume_rows 	= mysqli_num_rows($resume_result);
                    if($affected_resume_rows>0){
                      for($i=0;$i<$affected_resume_rows;$i++){
                        $row = mysqli_fetch_array($resume_result);
              ...
              break;
              case '3'://experience_level
                  if(isset($_GET['ex_id']) && !empty($_GET['ex_id'])){
                    $exp_id=mysql_fix_string(trim($_GET['ex_id']));
                    $query ="SELECT s.id,s.first_name,s.last_name,s.profile_url,u.join_date,s.sex,s.street,u.user_id FROM seeker AS s LEFT JOIN user AS u ON s.user_id=u.user_id LEFT JOIN  seeker_job_prefrence AS p ON p.seeker_id=s.id WHERE p.work_experience={$exp_id}";
                    $resume_result = $con->query($query);
                    $content ='<div class="row advert">';
                    $content .="<div class='h2'>Resumes with desired Employment Type</div>";
                    $affected_resume_rows 	= mysqli_num_rows($resume_result);
              ...
              case '4'://city
                  if(isset($_GET['c_id']) && !empty($_GET['c_id'])){
                    ...
                    $query ="SELECT s.id,s.first_name,s.last_name,s.profile_url,u.join_date,s.sex,s.street,u.user_id FROM seeker AS s LEFT JOIN user AS u ON s.user_id=u.user_id LEFT JOIN  seeker_job_prefrence AS p ON p.seeker_id=s.id WHERE p.city={$c_id}";
                    $resume_result = $con->query($query);
                    $content ='<div class="row advert">';
                    $content .="<div class='h2'>Resumes with desired Employment Type</div>";
                    $affected_resume_rows 	= mysqli_num_rows($resume_result);
              ...
              case '5'://province
                if(isset($_GET['p_id']) && !empty($_GET['p_id'])){
                  ...
                  $query ="SELECT s.id,s.first_name,s.last_name,s.profile_url,u.join_date,s.sex,s.street,u.user_id FROM seeker AS s LEFT JOIN user AS u ON s.user_id=u.user_id LEFT JOIN  seeker_job_prefrence AS p ON p.seeker_id=s.id WHERE p.province={$p_id}";
                  $resume_result = $con->query($query);
                  $content ='<div class="row advert">';
                  $content .="<div class='h2'>Resumes with desired Employment Type</div>";
                  $affected_resume_rows 	= mysqli_num_rows($resume_result);
