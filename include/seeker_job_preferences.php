        ...
        if(isset($_GET['inputCareerField']) && !empty($_GET['inputCareerField'])){
          if(isset($_GET['city']) && !empty($_GET['city'])){
            if(isset($_GET['inputProvince']) && !empty($_GET['inputProvince'])){
              ...
              $user_id = isset($_GET['inputUser_ID'])?mysql_fix_string(trim($_GET['inputUser_ID'])):null;
              //echo $salary;
              if(check_seeker_profile_dependeny($con,$user_id)){
                if(check_seeker_job_preference($con,$seeker_id)){
                  if(add_seeker_preference($con,$employment_type,$work_experience,$salary,$job_category,$city,$province,$seeker_id)){
                    ...
                    echo "Prefrence already set <em><a href='seeker_job_prefrences_update.php'>Update your prefrence instead</a>";
                }
              }else{
                    echo "System Error <em><a href='seeker_profile_edit.php'>Please Edit your profile before completing action...</a>";
              }
