...
$filter =" WHERE";
  if(isset($_GET['inputNaturalLanguageSearch']) && !empty($_GET['inputNaturalLanguageSearch'])){
    $keyword= mysql_fix_string(trim($_GET['inputNaturalLanguageSearch']));
    $filter .=" MATCH(j.attachment_content) AGAINST('{$keyword}')";
  }else{
    $title= "";
    $filter .=" MATCH(j.attachment_content) AGAINST('{$keyword}') OR 1=1";
  }
