...
include('../include/function_library.php');
 ...
 <?php
   ...
   $qualification_table ="<div><table class='table table-striped' border='0'>";
   $qualification_table .="<caption class='h3'>List of Qualifications</caption>";
   $qualification_table .="<tr><th>ID</th><th>Qualification</th></tr>";
   ...
   if($num_row>0){
     for($i=0;$i<$num_row;$i++){
       ...
       $result_flag=true;
       $id=$row['id'];
       $qualification_table .= "<tr><td>{$row['id']}</td><td>{$row['level']}</td</tr>";
     }
    $qualification_table .="</table></div>";
   }
 ?>
<!DOCTYPE html>
<html>
           ...
           <div class="row">
              <div class="col-md-6">
                <form method="post" action="<?php outputData($_SERVER['PHP_SELF']); ?>">
                      <input type="text" name="inputQualification" id="inputQualification" value="" placeholder="Enter Qualification Name" class="form-control"/><br />
                  <div class="form-inline">
                     <input type="submit" name="input_add_qualification" id="input_add_qualification" value="Add Qualification"  class="form-control btn btn-primary"/>
                 </div>
                </form>
                <?php
                    if(isset($_POST['input_add_qualification'])){
                      if(isset($_POST['inputQualification'])&&!empty($_POST['inputQualification'])){
                        ...
                        if(check_education_level($con,$table_name,$qualification)){
                          $error = "<div class='alert alert-danger'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>";
                          $error .= "<strong>System Error<br /></strong>Qualification already exists</div>";
                          echo $error;
                        }else{
                          if(add_education_level($con,$table_name,$qualification)){
                            $error = "<div class='alert alert-success'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>";
                            $error .= "<strong>Success<br /></strong>Qualification Added Successfully</div>";
                            echo $error;
                          }
                        }
                      }else{
                        $error = "<div class='alert alert-danger'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>";
                        $error .= "<strong>Input Error<br /></strong>Please Enter Qualification Name</div>";
                        echo $error;
                      }
                    }
                ?>
