...
include('../include/function_library.php');
 ...
 <?php
   ...
   $province_table ="<div><table class='table table-striped' border='0'>";
   $province_table .="<caption class='h3'>List of Provinces</caption>";
   $province_table .="<tr><th>ID</th><th>Province</th></tr>";
   ...
   if($num_row>0){
     for($i=0;$i<$num_row;$i++){
       ...
       $result_flag=true;
       $id=$row['id'];
       $province_table .= "<tr><td>{$row['id']}</td><td>{$row['name']}</td</tr>";
     }
    $province_table .="</table></div>";
   }
 ?>
<!DOCTYPE html>
<html>
           ...
           <div class="row">
              <div class="col-md-6">
                <form method="post" action="<?php outputData($_SERVER['PHP_SELF']); ?>">
                      <input type="text" name="inputProvince" id="inputProvince" value="" placeholder="Enter Province Name" class="form-control"/><br />
                  <div class="form-inline">
                     <input type="submit" name="input_add_province" id="input_add_province" value="Add Province"  class="form-control btn btn-primary"/>
                 </div>
                </form>
                <?php
                    if(isset($_POST['input_add_province'])){
                      if(isset($_POST['inputProvince'])&&!empty($_POST['inputProvince'])){
                        $province = mysql_fix_string(trim($_POST['inputProvince']));
                        $table_name="province";
                        if(check_data_insertion($con,$table_name,$province)){
                          ...
                          if(add_data_category($con,$table_name,$province)){
                            $error = "<div class='alert alert-success'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>";
                            $error .= "<strong>Success<br /></strong>Province Added Successfully</div>";
                            echo $error;
                          }
                        }
                      }else{
                        $error = "<div class='alert alert-danger'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>";
                        $error .= "<strong>Input Error<br /></strong>Please Enter Province Name</div>";
                        echo $error;
                      }
                    }
                ?>
