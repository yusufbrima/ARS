...
include('../include/function_library.php');
 ?>
 <?php
    $query =  "SELECT user_id,username,email,join_date,user_type,blocked FROM user ORDER BY username";
    $result_flag =false;
   $result = $con->query($query);
   $user_category="";
   $block_flag = "";
   $user_table ="<div><table class='table table-striped' border='0'>";
   $user_table .="<caption class='h3'>List of Users available</caption>";
   ...
   if($num_row>0){
     for($i=0;$i<$num_row;$i++){
       ...
       if($row['blocked']==0){
            $block_flag = "<a href='#' data-href='block_user.php?block_user_id={$id}' data-toggle='modal' data-target='#block_user_modal'><span class='glyphicon glyphicon-trash'></span> Block</a>";
       }elseif ($row['blocked']==1) {
            $block_flag = "<a href='#' data-href='block_user.php?unblock_user_id={$id}' data-toggle='modal' data-target='#unblock_user_modal'><span class='glyphicon glyphicon-trash'></span> Unblock</a>";
       }
       //<a href='#' data-href='block_user.php?block_user_id={$id}' data-toggle='modal' data-target='#block_user_modal'><span class='glyphicon glyphicon-trash'></span> Block</a>&nbsp;&nbsp;&nbsp;<a href='#' data-href='block_user.php?unblock_user_id={$id}' data-toggle='modal' data-target='#unblock_user_modal'><span class='glyphicon glyphicon-trash'></span> Unblock</a>
       $join_date = strtotime($row['join_date']);
       $output = date('l jS \of F Y h:i:s A',$join_date);
       $user_table .= "<tr><td>{$row['username']}</td><td>{$row['email']}</td><td>{$output}</td><td>{$user_category}&nbsp;&nbsp;&nbsp;$block_flag</td></tr>";
     }
...
<html>
       ...
       <div class="row"><!--Top Services of the Site-->
         <div class="container">
               <div class="row">
                 <div class="col-md-6">
                       <form>
                       <input type="search" name="inputBlockUsername" id="inputBlockUsername" value="" placeholder="Search by Username" class="form-control"/>
                     </form>
                 </div>
                 <div class="col-md-6">
                   <form>
                     <input type="search" name="inputBlockEmail" id="inputBlockEmail" value="" placeholder="Search by Email ID" class="form-control"/>
                   </form>
                 </div>
               </div>
  ...
  <?php include'block_user_modal.php'; ?><!--Script to block user in the system-->
  <?php include'unblock_user_modal.php'; ?><!--Script to block user in the system-->
  <?php include "footer.php"; ?><!--Site footer node -->
 <?php include "library.php"; ?><!--Script Library files -->
 </body>
 <script type="text/javascript">
 $('#block_user_modal').on('show.bs.modal', function(e) {
 $(this).find('.btn-ok').attr('href', $(e.relatedTarget).data('href'));
 });
 $('#unblock_user_modal').on('show.bs.modal', function(e) {
 $(this).find('.btn-ok').attr('href', $(e.relatedTarget).data('href'));
 });
 /*Natural Language Search for user names*/
   ...
   /*Natural Language Search for email id*/
     $('#inputBlockEmail').keyup(function(){
           //$('#feedback').append('a');
           $.get('include/email_block_search.php', {inputBlockEmail: $('#inputBlockEmail').val()},
               function(result){
                   $('#name_result').html(result).show();
                   $('#all_names').css('visibility','hidden');
               });
